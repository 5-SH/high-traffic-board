...
export default function () {
    ...
    range(parseInt(randomPage), 1).forEach(page => {
        const articleListResponse = http.get(`http://localhost:9005/v1/articles?boardId=1&pageSize=10&page=${page}`);
        ...
        articleIdList.forEach(id => {
            const articleResponse = http.get(`http://localhost:9005/v1/articles/${id}`);
