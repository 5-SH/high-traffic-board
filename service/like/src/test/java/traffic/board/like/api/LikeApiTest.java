package traffic.board.like.api;
...
public class LikeApiTest {
    ...
    ArticleLikeResponse read(Long articleId, Long userId) {
        return restClient.get()
                .uri("/v1/article-likes/articles/{artifactId}/users/{userId}", articleId, userId)
                .retrieve()
    ...
    void likePerformanceTest(ExecutorService executorService, Long articleId, String lockType) throws InterruptedException {
        ...
        for (int i = 0; i < 3000; i++) {
            long userId = i + 2;
            String finalLockType = lockType;
            executorService.submit(() -> {
                like(articleId, userId, finalLockType);
                latch.countDown();
            });
        }
